...
inflation_data <- read.csv(
  "https://raw.githubusercontent.com/canovasjm/inflacion/main/inflation_data.csv"
)
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      airDatepickerInput(
        ...
        label = "Mes inicio",
        value = seq(
          max(as.Date(inflation_data$date)),
          length = 2,
          by = "-11 months"
        )[2],
      ...
      h4(strong("Fuente")),
      HTML(
        "<p> INDEC, Dirección Nacional de Estadísticas de Precios, Dirección de Índices de Precios de Consumo. 
      ...
      h4(strong("Código")),
      HTML(
        "<p> <a href=https://github.com/canovasjm/inflacion > 
    ...
    mainPanel(
      ...
      fluidRow(
        column(
          width = 6,
          downloadButton(
            "download_inflation_monthly",
            "Descargar gráfico inflación mensual"
          ),
        ...
        column(
          width = 6,
          downloadButton(
            "download_inflation_cumulative",
            "Descargar gráfico inflación acumulada"
          ),
...
server <- function(input, output) {
  ...
  filter_data <- reactive({
    ...
    df <- inflation_data[
      inflation_data$date >= start_date & inflation_data$date <= end_date,
    ]
  ...
  output$inflation_table <- render_gt({
    ...
    table_data %>%
      gt() %>%
      fmt_percent(
        columns = c("inflacion_mensual", "inflacion_acumulada"),
        decimals = 2,
        scale_values = FALSE
      ...
      cols_label(
        ...
        inflacion_acumulada = "Inflacion acumulada"
      ) %>%
